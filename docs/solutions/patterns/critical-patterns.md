...
```bash
swift build && ./.build/debug/sharex --capture-selftest
# or
./build/ShareX.app/Contents/MacOS/ShareX
```

### ✅ CORRECT
```bash
./Scripts/make-app.sh          # bundles AND signs with a certificate identity
open build/ShareX.app          # LaunchServices makes the app its own TCC identity
