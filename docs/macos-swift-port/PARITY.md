...
| Auto-update (Release channel) | Ported — custom in-app updater, no Sparkle: checks the GitHub `releases/latest` API (manual "Check for Updates…" plus a Daily/Weekly/Monthly/Off background cadence), verifies the published sha256 and that the new bundle's Developer ID Team ID matches the running app, then atomically swaps the bundle and relaunches; opt-in auto-install. The update alert renders the release-notes markdown (headings, lists, inline styling) in a scrollable view, with "Skip This Version" as a checkbox. Homebrew installs are detected via the Caskroom and deferred to `brew upgrade --cask swiftx`. Keys are macOS-only (no C# counterpart to `UpdateChannel`/`AutoCheckUpdate`): `UpdateCheckFrequency`, `UpdateAutoInstall`, `UpdateSkippedVersion`, `UpdateLastCheckTime`. PreRelease/DevBuilds channels remain N/A |
