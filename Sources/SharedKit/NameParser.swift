    ...
    public func parse(_ pattern: String) -> String {
        ...
        for (entry, path) in entriesWithArgument(in: s, entry: "%rf") {
            s = replaceEachOccurrence(in: s, of: entry) { [isPreviewMode] in
                ...
                // ponytail: C# throws on upload and shows a popup; error surfacing lands with the task pipeline (Phase 2)
                return isPreviewMode ? "Valid text file path is required." : ""
            }
        }
...
enum Emoji {
    // ponytail: small curated set; C# Emoji.Emojis is the full unicode list - port it when %remoji parity matters
    static let emojis = ["😀", "😂", "😅", "😊", "😍", "😎", "😜", "🤔", "🤯", "😱", "🥳", "😴", "🐶", "🐱", "🦊", "🐼", "🦁", "🐸", "🐙", "🦄", "🌵", "🍀", "🌈", "⚡", "🔥", "❄️", "🌊", "⭐", "🍕", "🍩", "🎯", "🎲", "🎸", "🚀", "⚓", "🗿", "💎", "🧲", "🎁", "🔑"]
}
